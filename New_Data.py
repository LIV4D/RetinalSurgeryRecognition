...
new_train_direct = '/home/clement/CATARACTS/Training_zero/'
new_test_direct = '/home/clement/CATARACTS/Testing_zero/'
...
img_filenames = [path for path in img_filepath if read_phase(groundtruth_list,path) != 0] #Liste de toutes les images ['frame0', 'frame1', ...]

for path in img_filenames:
    head, tail = ntpath.split(path)
    direct, folder = ntpath.split(head)
    new_folder = new_train_direct + folder
...
for path in img_filenames:
    head, tail = ntpath.split(path)
    direct, folder = ntpath.split(head)
    new_folder = new_test_direct + folder
    create_folder(new_folder)
    destination = new_folder + '/' + tail 
    copyfile(path, destination)