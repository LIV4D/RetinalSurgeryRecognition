...
class MyNetwork_RNN(AbstractNet):
    ...
    def __init__(self, config):
        ...
        h0 = nn.Parameter(torch.zeros(2, 1, 512)) #was (2, 1, 10) pour les deux lignes #was (2, 1, 512)
        ...
        h0_RNN = nn.Parameter(torch.zeros(1, 1, self.config['CNN']['n_classes'])) #was (1, 1, 5) #was (1, 1, self.config['CNN']['n_classes'])
        h0_RNN = h0_RNN.cuda()
        self.h0_RNN = h0_RNN
        #pas forcément nécessaire
        
        self.LSTM = nn.LSTM(2048,1024,2) #was (1024,512,2) #was (19, 10, 2)
        self.RNN = nn.RNN(1024, self.config['CNN']['n_classes'], 1) #was (10, 5, 1) #was (512, self.config['CNN']['n_classes'], 1)
